...
public class Return implements Command {
    ...
    public String execute() {
        if (!API.visitorIsRegisteredID(visitorID)) return "invalid-visitor-id;";
        Visitor visitor = API.getVisitorByID(visitorID);

        if (visitor.getNumCheckedOut() < ids.size()) return "invalid-book-id," +
                ids.toString().substring(1, ids.size()-1).replaceAll("\\s", "") + ";";
