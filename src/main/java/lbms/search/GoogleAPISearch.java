...
public final class GoogleAPISearch {
    ...
    public static List<Book> searchByTitle(String title) {
        return parseJSON( query(baseURL + "intitle:\"" + title.replaceAll(" ", "+") + "\"" + saleableSuffix + maxResultsSuffix) );
    }
        ...
        for(int i = 0; i < books.size(); i++) {
            try {
                ...
                if(volumeInfo.get("industryIdentifiers") == null) {
                    ...
                    JsonObject isbnInfo = volumeInfo.get("industryIdentifiers").getAsJsonArray().get(0).getAsJsonObject();
