...
public class BorrowBookViewState implements State {
    ...
    @Override
    public void onEnter() {
        String response = new ProxyCommandController().processRequest("borrow," + this.visitorID + this.books +";");
    ...
    public String parseResponse(String response) {
        ...
        switch (fields[1]) {
            ...
            case "outstanding-fine":
                return "\nVisitor " + this.visitorID + " has to pay " +
                        new DecimalFormat("#.00").format(UserSearch.BY_ID.findFirst(this.visitorID).getFines()) +
                        " before they " + "can borrow more books.";
            case "book-limit-exceeded;":
                return "\nVisitor " + this.visitorID + " has borrowed the maximum number of books or the borrow " +
                        "request would cause the visitor to exceed 5 borrowed books.";
            case "invalid-book-id":
                return "\nOne of more of the book IDs specified do not match the IDs for the most recent library " +
                        "book search.";
            default:
